...
from nets.SPFNet1 import EGCA, Adjustment,SPFM,DSCModule,conv_block
...
class GlobalContext(nn.Module):
    def __init__(self, backbone='STDCNet1446', pretrain_model='STDCNet1446_76.47.tar', use_conv_last=True, *args, **kwargs):
        ...
        if backbone == 'STDCNet1446':
            ...
            self.arms1 = FeatureFusionBlock(640, 128)
            self.arms2 = FeatureFusionBlock(384, 128)
            self.attention = CoordAtt(1024, 1024)
            ...
            self.conv_head = ConvBNReLU(128, 128, ks=3, stride=1, padding=1)
            self.conv_avg = ConvBNReLU(inplanes, 128, ks=1, stride=1, padding=0)
            self.conv_head0 = ConvBNReLU(1024, 128, ks=3, stride=1, padding=1)
    ...
    def forward(self, in_f):
        ...
        feat2, feat4, feat8, feat16, feat32 = self.backbone(in_f)
        H8, W8 = feat8.size()[2:]
        ...
        feat32_arms=self.attention(feat32)
        feat32_arms=self.conv_head0(feat32_arms)
        feat32_arm = self.arm32(feat32)
        feat32_sum = feat32_arm + avg_up+ feat32_arms
        feat32_up = F.interpolate(feat32_sum, (H16, W16), mode='bilinear', align_corners=True)
        feat32_up = self.conv_head32(feat32_up)

        feat16_sum = self.arms1(feat16, feat32_up)
        feat16_arm = self.arm16(feat16)
        feat16_sum = feat16_arm + feat32_up+feat16_sum
        feat16_up = F.interpolate(feat16_sum, (H8, W8), mode='bilinear', align_corners=True)
        feat16_up  = self.conv_head16(feat16_up)


        feat8_sum = self.arms2(feat8, feat16_up)
        feat8_arm = self.arm8(feat8)
        feat8_sum = feat8_arm +feat16_up + feat8_sum

        feat8_up = F.interpolate(feat8_sum, (H4, W4), mode='bilinear', align_corners=True)
        feat8_up = self.conv_head(feat8_up)



        return feat2, feat4, feat8, feat16, feat8_up, feat16_up, feat32_up
...
class BiSeNet(nn.Module):
    def __init__(self, backbone, n_classes, pretrain_model='', use_boundary_2=False, use_boundary_4=False,
        ...
        self.conv_out32 = BiSeNetOutput(conv_out_inplanes, 64, 128)
...
if __name__ == "__main__":
    net = BiSeNet('STDCNet813', 5)
    net.cuda()
    net.eval()
    in_ten = torch.randn(6, 3, 240, 427).cuda()
