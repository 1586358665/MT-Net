...
def multibatch_collate_fn(batch):
    ...
    try:
        ...
        info = None
#batch中含有 图片矩阵，图片对应的mask，调色盘，图片的相关信息
...
#oh size 720*1280*9 里面为bool变量（当最大目标为8时）
    return oh

def convert_one_hot(oh, max_obj):

    mask = np.zeros(oh.shape[:2], dtype=np.uint8) #取oh的前两个维度，720*1280，形成一个720*1280的0矩阵
    for k in range(max_obj+1):
        mask[oh[:, :, k]==1] = k
...
class VIL(BaseData):
    ...
    def __getitem__(self, idx):
        ...
        frames = [name[:5] for name in os.listdir(annofolder)]   #从数据集中取出图片名称
        frames.sort()
        nframes = len(frames)
        ...
        if self.train:
            ...
            for i in range(nsamples):
                if i == 0:
                    last_sample = random.randrange(0,nframes - nsamples + 1,1)
        ...
        #     sample_frame = []
        #
        #     nsamples = min(self.sampled_frames, nframes)  # 每隔一个帧，共取10个
        #     for i in range(nsamples):
        #         if i == 0:
        #             last_sample = random.sample(range(0, 80), 1)[0]
        #         else:
        #             last_sample += 2
        #         sample_frame.append(frames[last_sample])
