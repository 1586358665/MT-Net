...
class Compose(object):
    ...
    def __init__(self, transform=[]):              #transform为一个列表把所有的相关转换放入列表中
        self.transforms = transform

    def __call__(self, imgs, annos, use_image):

        for m in self.transforms:
            imgs, annos = m(imgs, annos, use_image)    #将变量赋值到transforms中的变换中去
...
class Transpose(object):
    ...
    def __call__(self, imgs, annos, use_image):
        ...
        if H < W:
            ...
            return timgs, tannos           #改变图片高和宽位置确保h小于w
...
class Rescale(object):
    ...
    def __init__(self, target_size):
        assert isinstance(target_size, (int, tuple, list))   #判断输入size的形式
        if isinstance(target_size, int):                       #是整数时，就直接把图片大小设为 N*N
            self.target_size = (target_size, target_size)
        else:
            self.target_size = target_size           #不为整数就按原来样子保存

    def __call__(self, imgs, annos, use_image):

        h, w = imgs[0].shape[:2]                          #数据中图片大小
