...
class SPFNet(nn.Module):
    def __init__(self, backbone="resnet18", pretrained=True, classes=9):
        ...
        self.adjust3 = Adjustment(128, 128)
        self.adjust4 = Adjustment(256, 128)
        self.adjust5 = Adjustment(512, 128)

        # Multi-Scale feature fusion
        self.fuse = conv_block(576, 64, 3, 1, padding=1, bn_act=True)
        ...
        self.classifier1 = Classifier(64,classes)
    ...
    def forward(self, x):
        ...
        adj2s = F.interpolate(adj2, size=x2.size()[2:], mode='bicubic', align_corners=True)
        adj3s = F.interpolate(adj3, size=x2.size()[2:], mode='bicubic', align_corners=True)
        adj4s = F.interpolate(adj4, size=x2.size()[2:], mode='bicubic', align_corners=True)
        adj5s = F.interpolate(adj5, size=x2.size()[2:], mode='bicubic', align_corners=True)
        dsc2s= F.interpolate(dsc2, size=x2.size()[2:], mode='bicubic', align_corners=True)
      

        msfuse = torch.cat((adj2s,dsc2s,adj3s,adj4s,adj5s), dim=1)
        msfuse = self.fuse(msfuse)

        classifier = self.classifier1(msfuse)
        classifier = F.interpolate(classifier, size=(H, W), mode='bicubic', align_corners=True)

        return adj2,adj3,adj4,classifier
...
class RPPModule(nn.Module):
    def __init__(self, in_channels: int, groups=2) -> None:
        ...
        self.groups = groups
        self.conv_dws11 = nn.Sequential(
            conv_block(in_channels, 4*in_channels, kernel_size=3, stride=1, padding=1, group=1, bn_act=True),
            nn.PixelShuffle(upscale_factor=2))
        self.conv_dws21 = nn.Sequential(
            conv_block(in_channels, 4*in_channels, kernel_size=3, stride=1, padding=1, group=1, bn_act=True),
            nn.PixelShuffle(upscale_factor=2))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        br1 = self.conv_dws11(x)
        b2 = self.conv_dws11(x)

        out = torch.cat((br1, b2), dim=1)
        out = self.fusion(out)

        br3 = self.conv_dws3(self.avg(x))
        output = br3 + out
...
class EGCA(nn.Module):
    def __init__(self, in_channels: int, groups) -> None:
...
#     input_tensor = torch.rand(2, 3, 240, 448)
